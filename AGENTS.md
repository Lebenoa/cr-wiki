...
- **Grade model** (`database/models/grade.v`): `enum Grade as u8` ordered `e, c, b, a, s, s_plus, l` — E ("Extra") ranks ABOVE L ("Legend"). Ordering comes from `grade_values`; labels are just `g.to_upper()` (no `grade_name()`). V enums convert via `.from('x')` / `.from(1)` and `.str()`. `treasure.grade` is `?int` — `none` means no wiki grade, no badge.
